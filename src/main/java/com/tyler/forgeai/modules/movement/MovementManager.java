...
public class MovementManager {
    ...
    public void tick(ContextScanner.Signals s) {
        if (!active || s == null || s.player == null) return;
        // Apply small jitter to look and movement to appear human-like
        try {
            if (rng.nextInt(10) == 0) {
                double yawJitter = (rng.nextDouble() - 0.5) * 6.0; // +/- 3 degrees
                double pitchJitter = (rng.nextDouble() - 0.5) * 2.0;
            ...
            // If in elytra flight, occasionally do minor course corrections
            if (s.isFlyingWithElytra()) {
                if (rng.nextInt(10) < 3) {
                    com.tyler.forgeai.util.PlayerActionUtils.adjustLook(s.player, (float) ((rng.nextDouble() - 0.5) * 12.0), 0f);
                }
    ...
    public void moveTo(ServerPlayer player, BlockPos pos) {
        try {
            com.tyler.forgeai.util.PlayerActionUtils.navigateTo(player, pos);
        } catch (Exception e) { LOGGER.warn("moveTo failed: {}", e.getMessage()); }
    }

    public void strafe(ServerPlayer player, float strength) {
        try { com.tyler.forgeai.util.PlayerActionUtils.strafe(player, strength); } catch (Exception e) {}
    }

    public void dodge(ServerPlayer player) {
        try { com.tyler.forgeai.util.PlayerActionUtils.dodge(player); } catch (Exception e) {}
    }

    public void startElytra(ServerPlayer player) {
        try { com.tyler.forgeai.util.PlayerActionUtils.useElytra(player); } catch (Exception e) {}
    }

    public void stopElytra(ServerPlayer player) {
        try { com.tyler.forgeai.util.PlayerActionUtils.cancelElytra(player); } catch (Exception e) {}
    }
}
